...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	$sql = "SELECT * from user_exp inner join users on users.usr_id = user_exp.users_usr_id join user_lvl on user_lvl.users_usr_id = users.usr_id WHERE usr_country = '" . ($country_code) . "' ORDER BY user_exp.exp_current desc limit 0,10";
...
$local_rank_query = "SELECT users_usr_id from user_exp 
    ...
    while($local_rank_query_row = mysqli_fetch_array($execute_local_rank_query)){
        ...
        $local_position = array_search($usr_id, $local_rank) + 1;
