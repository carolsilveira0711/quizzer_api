...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($now_formatted > $time_formatted && $row['user_itm_lives'] == 0) {
        ...
        SET user_itm_lives = '" . ($lives) . "' where users_usr_id ='" . ($usr_id) . "'";
    ...
    $next_exp_needed = mysqli_query($conn, "SELECT exp_need FROM level WHERE lvl_id = '" .  $next_lvl . "'");

	$next_exp_needed_array = mysqli_fetch_assoc($next_exp_needed);

	$next_user_exp = $next_exp_needed_array['exp_need'];
    ...
    $user_items_select = mysqli_query($conn, "SELECT items_itm_id, SUM(user_itm_amount), user_itm_name 
                                              FROM user_itm 
                                              WHERE users_usr_id = '$usr_id' 
